...
def save_state(state, sha):
    """Write state atomically. On any conflict, merge with the latest and retry."""
    _prune(state)
    if not USE_API:
    ...
    for attempt in range(5):
        ...
        if r.status_code in (409, 422):  # sha stale -> someone else wrote; merge + retry
            ...
            if g.status_code == 200:
                ...
                _prune(state)
                continue
        log("save_state failed:", r.status_code, r.text[:200])
        return False
    log("save_state: gave up after retries")
    return False
...
def main():
    ...
    try:
        ...
        # If we cannot read the latest memory, do NOT notify (would risk duplicates).
    ...
    if first_run:
        ...
        if save_state(state, sha):
            ...
            log("First-run save failed; will seed again next run (no message sent).")
        return

    _, notified = process(items, False, seen)
    if not save_state(state, sha):
        log("WARNING: save failed after notifying; ids may repeat next run.")
